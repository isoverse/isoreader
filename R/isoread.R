...
#' @param cache whether to cache iso_files. Note that previously exported R Data Archives (di.rds, cf.rds) are never cached since they are already essentially in cached form.
...
#' @details \code{iso_reread_archive} is a convenience function for refreshing saved iso_file collections. It will load a specific iso_files R Data Archive (\code{rds_filepath}), re-read all the data from the original data files and save the collection back to the same rds file. The iso_files are returned invisibly.
...
#' @export
iso_reread_archive <- function(rds_filepaths, ..., stop_if_missing = FALSE, quiet = default(quiet)) {

  extensions <- iso_get_supported_file_types() %>% dplyr::filter(stringr::str_detect(extension, "rds"))
  ...
  reread_archive <- function(filepath, call) {
    ...
    suppressWarnings(do.call(call, args = list(paths = filepath, quiet=TRUE))) %>%
      ...
      iso_export_to_rds(filepath = filepath, quiet=quiet)
    return(TRUE)
  }
