...
isoread_did <- function(ds, ...) {
  ...
  if (ds$read_option$raw_data)
    ds <- exec_func_with_error_catch(process_did_raw_voltage_data, ds)
...
# process voltage data in did file
process_did_raw_voltage_data <- function(ds) {
  # find masses
  ...
  masses <- fetch_keys(
    ds$binary, "Mass \\d+", byte_min = CTraceInfo$byte_end, byte_max = CPlotRange$byte_start, require = "1+") %>% 
    mutate(
      # column names
  ...
  read_voltage_data <- function(byte_start, byte_end) {
    ...
    # retrieve data
    parse_binary_data(ds$binary, "double", length = nrow(masses), sensible = c(-1000, 100000)) %>% 
      as.list() %>% 
