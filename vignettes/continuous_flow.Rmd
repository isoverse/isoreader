...
cf_files2 %>% iso_filter_files(grepl("linearity", ID)) %>% 
  iso_get_file_info() %>% 
  rmarkdown::paged_table()
...
The file information in any collection of isofiles can also be mutated using the function `iso_mutate_file_info`. This function can introduce new columns and operate on any existing columns available in the file information (even if it does not exist in all files) and supports full [dplyr](https://dplyr.tidyverse.org/reference/mutate.html) syntax. It can also be used in conjuction with `iso_with_unit` to generate values with implicit units.
