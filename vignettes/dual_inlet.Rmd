...
iso_files %>% 
  iso_get_file_info(
    select = c(
      ...
      # select the Analysis column (not changing the name)
      Analysis,
...
iso_files2 <- iso_files %>% 
  iso_select_file_info(
    ID = `Identifier 1`, ID = `Sample Name`, Analysis, `Date & Time` = file_datetime
  )
...
iso_files2 %>% iso_filter_files(grepl("CIT", ID)) %>% 
  iso_get_file_info() %>% 
  rmarkdown::paged_table()
...
iso_files2 %>% 
  iso_mutate_file_info(
    ...
    ID = paste("ID:", ID),
    # change type of Analysis column to number
