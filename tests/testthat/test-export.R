...
test_that("test that export to Excel works properly", {
  expect_error(export_to_excel(42), "can only export iso files")
  expect_error(export_to_excel(isoreader:::make_cf_data_structure()), "no filepath provided")
  expect_error(export_to_excel(isoreader:::make_cf_data_structure(), file.path("DOESNOTEXIST", "test")), 
               "folder .* does not exist")
  ...
  # export and check
  expect_message(cf_out <- export_to_excel(cf, filepath, quiet = FALSE), "exporting data .* into Excel")
  expect_equal(cf, cf_out)
