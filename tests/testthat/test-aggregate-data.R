...
test_that("test that aggregating file info works", {
  ...
  iso_file <- isoreader:::make_iso_file_data_structure()
  expect_warning(iso_aggregate_file_info(iso_file), "read without extracting the file info")
  ...
  expect_message(iso_aggregate_file_info(c(iso_file1, iso_file2), quiet = FALSE), "aggregating")
  expect_silent(agg <- iso_aggregate_file_info(c(iso_file1, iso_file2), quiet = TRUE))
  expect_equal(names(agg), unique(names(iso_file1$file_info), names(iso_file2$file_info)))
  expect_equal(iso_aggregate_file_info(c(iso_file1, iso_file2)), 
               {
  ...
  expect_equal(names(iso_aggregate_file_info(iso_file1, select = c("file_datetime", "only_a"))), c("file_id", "file_datetime", "only_a"))
  expect_warning(agg <- iso_aggregate_file_info(iso_file2, select = c("file_datetime", "only_a")), "file info entries do not exist")
...
test_that("test that aggregeting raw data works", {
  ...
  iso_file <- isoreader:::make_iso_file_data_structure()
  expect_warning(iso_aggregate_raw_data(iso_file), "read without extracting the raw data")
  ...
  expect_message(iso_aggregate_raw_data(c(iso_file1, iso_file2), quiet = FALSE), "aggregating")
  expect_silent(iso_aggregate_raw_data(c(iso_file1, iso_file2), quiet = TRUE))
  expect_equal(iso_aggregate_raw_data(c(iso_file1, iso_file2)), 
               data <- bind_rows(mutate(iso_file1$raw_data, file_id="a"), 
                                 mutate(iso_file2$raw_data, file_id = "b")))
  
  expect_equal(iso_aggregate_raw_data(c(iso_file1, iso_file2), gather = TRUE), 
  ...
  expect_true("test_info" %in% names(agg <- iso_aggregate_raw_data(c(iso_file1, iso_file2), include_file_info = c("test_info"))))
...
test_that("test that aggregating of methods standards works", {
  ...
  iso_file <- isoreader:::make_iso_file_data_structure()
  expect_warning(iso_aggregate_standards_info(iso_file), "read without extracting the method info")
  ...
  expect_message(iso_aggregate_standards_info(c(iso_file1, iso_file2), quiet = FALSE), "aggregating")
  expect_silent(iso_aggregate_standards_info(c(iso_file1, iso_file2), quiet = TRUE))
  expect_equal(iso_aggregate_standards_info(c(iso_file1, iso_file2)), 
               data <- bind_rows(mutate(iso_file1$method_info$standards, file_id="a"), 
                                 mutate(iso_file2$method_info$standards, file_id="b")))
  # include file info
  iso_file1 <- modifyList(iso_file1, list(file_info = list(test_info = "x")))
  iso_file2 <- modifyList(iso_file2, list(file_info = list(test_info = "y")))
  expect_true("test_info" %in% names(agg <- iso_aggregate_standards_info(c(iso_file1, iso_file2), include_file_info = c("test_info"))))
  expect_equal(unique(agg$test_info), c("x", "y"))
...
test_that("test that aggregating of resistors works", {
  ...
  iso_file <- isoreader:::make_iso_file_data_structure()
  expect_warning(iso_aggregate_resistors_info(iso_file), "read without extracting the method info")
  ...
  expect_message(iso_aggregate_resistors_info(c(iso_file1, iso_file2), quiet = FALSE), "aggregating")
  expect_silent(iso_aggregate_resistors_info(c(iso_file1, iso_file2), quiet = TRUE))
  expect_equal(iso_aggregate_resistors_info(c(iso_file1, iso_file2)), 
               data <- bind_rows(mutate(iso_file1$method_info$resistors, file_id="a"), 
                                 mutate(iso_file2$method_info$resistors, file_id="b")))
  # include file info
  iso_file1 <- modifyList(iso_file1, list(file_info = list(test_info = "x")))
  iso_file2 <- modifyList(iso_file2, list(file_info = list(test_info = "y")))
  expect_true("test_info" %in% names(agg <- iso_aggregate_resistors_info(c(iso_file1, iso_file2), include_file_info = c("test_info"))))
  expect_equal(unique(agg$test_info), c("x", "y"))
...
test_that("test that aggregating of vendor data table works", {
  ...
  iso_file <- isoreader:::make_iso_file_data_structure()
  expect_warning(iso_aggregate_vendor_data_table(iso_file), "read without extracting the vendor data table")
  ...
  # unit information
  expect_warning(iso_aggregate_vendor_data_table(iso_file1, with_units = TRUE), "do not have unit information")
  expect_message(iso_aggregate_vendor_data_table(iso_file1, with_units = FALSE, quiet = FALSE), "aggregating")
  expect_silent(iso_aggregate_vendor_data_table(iso_file1, with_units = FALSE, quiet = TRUE))
  ...
  expect_message(agg <- iso_aggregate_vendor_data_table(c(iso_file1, iso_file2), with_units = TRUE, quiet = FALSE), "aggregating")
  expect_equal(agg, 
  ...
  expect_equal(iso_aggregate_vendor_data_table(c(iso_file1, iso_file2), with_units = FALSE), 
               bind_rows(mutate(iso_file1$vendor_data_table, file_id="a"),
                         mutate(iso_file2$vendor_data_table, file_id="b")))
  ...
  iso_file1 <- modifyList(iso_file1, list(file_info = list(test_info = "x")))
  iso_file2 <- modifyList(iso_file2, list(file_info = list(test_info = "y")))
  expect_true("test_info" %in% names(agg <- iso_aggregate_vendor_data_table(c(iso_file1, iso_file2), include_file_info = c("test_info"))))
  expect_equal(unique(agg$test_info), c("x", "y"))
