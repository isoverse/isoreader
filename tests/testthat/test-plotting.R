...
test_that("test that plot continuous flow works properly", {
  ...
  expect_error(plot_raw_data(cf, masses = c(), ratios = c()), "must specify at least one mass or ratio")
  expect_error(plot_raw_data(cf, ratios = c("42")), "invalid ratio")
  expect_error(plot_raw_data(cf, ratios = c("3/2")), "mass\\(es\\) not available in the provided isofiles")
  ...
  expect_error(plot_raw_data(cf, panels = "42"), "unknown layout specification")
  expect_error(plot_raw_data(cf, colors = "42"), "unknown layout specification")
  expect_error(plot_raw_data(cf, linetypes = "42"), "unknown layout specification")
  expect_is(p <- plot_raw_data(cf, ratios = c("46/44")), "ggplot")
