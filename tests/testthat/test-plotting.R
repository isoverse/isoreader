...
test_that("test that plot continuous flow works properly", {
  ...
  expect_is(cf <- make_cf_data_structure() %>% 
              update_read_options(read_file_info = TRUE, read_raw_data = TRUE), "continuous_flow")
...
test_that("test that plot dual inlet works properly", {
  ...
  expect_is(di <- make_di_data_structure() %>% update_read_options(read_raw_data = TRUE, read_file_info = TRUE), "dual_inlet")
